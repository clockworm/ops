...
TreeGrid = function(_config){
	...
	drowHeader = function(){
		...
		s += "<thead><tr class='' height='" + (_config.headerHeight || "25") + "'>";
		var cols = _config.columns;
		for(i=0;i<cols.length;i++){
			var col = cols[i];
			if(col.hidden)
			s += "<th style='display:none;text-align:" + (col.headerAlign || _config.headerAlign || "center") + "' width='" + (col.width || "") +"'>" + (col.headerText || "") + "</th>";
			else
			s += "<th style='text-align:" + (col.headerAlign || _config.headerAlign || "center") + "' text-align='" + (col.headerAlign || _config.headerAlign || "center") + "' width='" + (col.width || "") +"'>" + (col.headerText || "") + "</th>";
		}
		s += "</tr></thead>";
	}
	...
	this.show = function(){
		...
		s += "<table id='" + this.id + "' cellspacing=0 cellpadding=0 width='" + (_config.width || "100%") + "' class='"+(_config.skin||'')+"'>";
		drowHeader();
		drowData();
		s += "</table>";
...
//公共静态变量
TreeGrid.FOLDER_OPEN_ICON = "images/folderOpen.png";
TreeGrid.FOLDER_CLOSE_ICON = "images/folderClose.png";
TreeGrid.DEFAULT_LEAF_ICON = "images/defaultLeaf.png";
TreeGrid.COUNT = 1;
